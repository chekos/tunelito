...
When a folder URL has no authored `index.html` or `index.md`, Tunelito serves an intentional folder landing page instead of a raw listing. It shows only the immediate served documents and child folders, distinguishes their types, provides a parent link for nested folders, and uses the active Markdown theme without writing an index into the source tree.
