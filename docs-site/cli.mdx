...
The same process serves the room, watches comments, claims actionable feedback, and prints a prompt for the current agent. Record the current agent's result with the claim id from the printed command:
