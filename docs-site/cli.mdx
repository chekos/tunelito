...
Run Codex against actionable comments:

```bash
tunelito ./site --agent codex --agent-trigger all
```

Run Claude Code against comments containing `@agent`:
...
```bash
tunelito ./site --agent-command "openclaw run --stdin" --agent-trigger all
```
