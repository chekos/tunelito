...
When `--agent-session` is enabled, Tunelito does not invoke another CLI. It writes `.tunelito/session.json` and prints inbox commands so the current coding-agent session can claim comments with `tunelito inbox watch` and record results with `tunelito inbox record`.
