...
component: tooling
symptoms:
  - "Persistent Japan trip launchd job ran node bin/tunelito.js from the repo instead of the released package path"
...
The long-running Japan trip review server was initially made reliable at the process level, but not at the release-path level. The `launchd` job ran the repository checkout directly with `node bin/tunelito.js`, while the trip workflow should have exercised the released package entrypoint users actually run: `npx tunelito`.
...
- The default Claude provider command did not allow web tools, so a comment asking for specific venue links could only be partially handled.
...
- Persistent service target: Japan 2026 itinerary folder in the local Obsidian vault
...
```text
/opt/homebrew/bin/node /Users/chekos/projects/gh/tunelito/bin/tunelito.js ...
```
...
```text
https://sergios-macbook-pro-1.tail7a6776.ts.net/
|-- proxy http://100.102.216.65:4317
```
...
- **Why it failed:** It kept the trip server alive, but it did not verify the packaged CLI. A broken `bin` entry, missing packed file, or executable permission regression could still pass the local trip setup.
...
- **Why it was incomplete:** Adding `/Users/chekos/.local/bin` made `claude` discoverable, but it still did not match how the user runs tools from a real terminal. The durable approach is to run the launchd command through `zsh -lic`.
...
### 2. Run the trip service from the released package path

The `launchd` job was changed to invoke the released package from a clean working directory:

```bash
cd /private/tmp && exec npx --yes tunelito@0.4.1 /Users/chekos/projects/amox/projects/japan-trip-2026 \
  --host 127.0.0.1 \
  --port 4317 \
  --no-tunnel \
  --no-auth \
  --out /Users/chekos/projects/amox/projects/japan-trip-2026/.tunelito/comments.md \
  --agent custom \
  --agent-command '/Users/chekos/.local/bin/claude -p --permission-mode acceptEdits --output-format json --allowedTools Read,Write,Edit,MultiEdit,LS,Grep,Glob,WebSearch,WebFetch --add-dir /Users/chekos/projects/amox/projects/japan-trip-2026' \
  --agent-state /Users/chekos/projects/amox/projects/japan-trip-2026/.tunelito/agent-state.json \
  --agent-interval 120 \
  --agent-instructions "..."
```
...
- `--agent custom` permits a provider command with the exact Claude tools needed for this trip.
...
{
  "Web": {
    "sergios-macbook-pro-1.tail7a6776.ts.net:443": {
      "Handlers": {
        "/": {
...
```text
wss://sergios-macbook-pro-1.tail7a6776.ts.net/__tunelito/ws?tunelito_page=/itinerary-day-03.html
```
...
{
  "status": "ignored",
  "summary": "HTTPS verification comment only; no itinerary change requested. Confirmed target page itinerary-day-03.html and quoted text exist. Tailscale HTTPS WebSocket path reached the agent worker.",
  "filesChanged": []
}
```

The test comments were removed afterward so the itinerary review inbox stayed clean.
...
- If comments are expected to request current links or venues, the agent command must allow web tools and the instructions must forbid fabricated URLs.
